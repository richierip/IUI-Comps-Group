...
def BFS(xy1, xy2, gameStateData, ignore_spaces=None):
    # Spots on the board that will not be considered when doing BFS
    if ignore_spaces is None:
        ignore_spaces = []
    else:
        if xy1 in ignore_spaces:
            ignore_spaces.remove(xy1)
        if xy2 in ignore_spaces:
            ignore_spaces.remove(xy2)
    ...
    while not q.isEmpty() and not done:
        ...
        for neighbor in neighbors:
            if neighbor not in seen and neighbor not in ignore_spaces:
                q.push(neighbor)
